...
export function ListingWizard() {
  ...
  return (
    <form action={action} className="border border-[#1D1B16] bg-[#0E0D0B]">
      ...
      {/* stepper */}
      <div className="flex flex-wrap gap-2 border-b border-[#1D1B16] px-5 py-4">
        {STEPS.map((s, i) => (
          <span key={s} className={`text-[9px] font-semibold uppercase tracking-[0.14em] ${i === step ? "text-[#C9A227]" : i < step ? "text-[#7E9877]" : "text-[#4E4840]"}`}>
            {i + 1}. {s}
          </span>
        ))}
      </div>
      ...
      <div className="min-h-[280px] p-6">
        ...
        {step === 5 && (
          <Field label="Main photo URL">
            ...
            <p className="mt-2 text-[12px] text-[#8A8074]">In production this is a drag-and-drop uploader with automatic thumbnail generation. For now, paste an image URL (optional).</p>
          </Field>
        )}
        {step === 6 && (
          <>
        ...
        {step === 8 && (
          <div>
            ...
            <dl className="grid gap-px bg-[#1D1B16]">
              {[
                ["Title", data.title || "—"],
                ...
                <div key={k} className="flex justify-between gap-4 bg-[#0B0A08] px-4 py-3 text-[13.5px]">
                  <span className="text-[#8A8074]">{k}</span>
                  <span className="text-right">{v}</span>
                </div>
              ))}
            </dl>
            <p className="mt-4 text-[12px] text-[#8A8074]">On publish, the listing is submitted for admin review. It goes live once approved.</p>
          </div>
        )}
        {state.error && <p className="mt-4 text-[13px] text-[#E2712B]">{state.error}</p>}
      </div>

      <div className="flex items-center justify-between border-t border-[#1D1B16] px-6 py-4">
        <button type="button" onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0} className="text-[12px] font-semibold text-[#8A8074] disabled:opacity-40">
          ← Back
        </button>
        {step < STEPS.length - 1 ? (
          <button type="button" onClick={() => canNext() && setStep((s) => s + 1)} disabled={!canNext()} className="border border-[#C9A227] bg-[#C9A227] px-6 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#0B0A08] disabled:opacity-40">
            Continue →
          </button>
        ) : (
          <Publish />
...
function Publish() {
  ...
  return (
    <button type="submit" disabled={pending} className="border border-[#C9A227] bg-[#C9A227] px-6 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#0B0A08] disabled:opacity-60">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-4">
      <div className="mb-2 text-[9px] font-semibold uppercase tracking-[0.16em] text-[#8A8074]">{label}</div>
      {children}
    </div>
  );
}
...
function Choice({ active, onClick, children }: { active?: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-full px-4 py-2.5 text-[13px] font-semibold ${active ? "bg-[#C9A227] text-[#0B0A08]" : "border border-[#33302A] text-[#F4F0E6]/80 hover:border-[#C9A227]"}`}>
      {children}
    </button>
  );
}
