              ...
              <div key={s.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                <div>
                  ...
                  <div className="text-[11.5px] text-[#8AA0B4]">
                    {s.adapter} · {s.authorised ? <span className="text-[#5FA39C]">authorised</span> : <span className="text-[#E2712B]">not authorised</span>} · {s.automated ? s.schedule?.toLowerCase() : "manual"}
                ...
                {s.authorised && s.adapter !== "MANUAL_URL" && (
                  <form action={runImportAction}>
                    ...
                    <button className="border border-[#274563] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] hover:border-[#C6A15C] hover:text-[#C6A15C]">Sync</button>
                  </form>
                )}
              </div>
            ))}
            ...
            {jobs.map((j) => (
              <div key={j.id} className="py-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-semibold uppercase" style={{ color: j.status === "FAILED" ? "#C05B4A" : j.status === "COMPLETED" ? "#5FA39C" : "#C6A15C" }}>{j.status}</span>
                </div>
                <div className="mt-1 text-[11.5px] text-[#8AA0B4]">
                  {j.discovered} discovered · {j.updated} queued · {j.duplicates} dup · {j.errors} err · {timeAgo(j.createdAt)}
                </div>
              </div>
            ))}
            {jobs.length === 0 && <p className="py-3 text-[13px] text-[#8AA0B4]">No jobs yet.</p>}
          </div>
      ...
      <div className="mt-6">
        ...
        {records.length === 0 ? (
          <div className="border border-[#1A3550] bg-[#0D2540] p-10 text-center">
            ...
            <p className="mt-2 text-[13px] text-[#8AA0B4]">Imports appear here for approval before they can go live.</p>
          </div>
        ) : (
