              ...
              <div key={s.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                <div>
                  ...
                  <div className="text-[11.5px] text-[#95A79C]">
                    {s.adapter} · {s.authorised ? <span className="text-[#6FB58F]">authorised</span> : <span className="text-[#E2712B]">not authorised</span>} · {s.automated ? s.schedule?.toLowerCase() : "manual"}
                ...
                {s.authorised && s.adapter !== "MANUAL_URL" && (
                  <form action={runImportAction}>
                    ...
                    <button className="border border-[#245140] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] hover:border-[#D6A84F] hover:text-[#D6A84F]">Sync</button>
                  </form>
                )}
              </div>
            ))}
            ...
            {jobs.map((j) => (
              <div key={j.id} className="py-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-semibold uppercase" style={{ color: j.status === "FAILED" ? "#C05B4A" : j.status === "COMPLETED" ? "#6FB58F" : "#D6A84F" }}>{j.status}</span>
                </div>
                <div className="mt-1 text-[11.5px] text-[#95A79C]">
                  {j.discovered} discovered · {j.updated} queued · {j.duplicates} dup · {j.errors} err · {timeAgo(j.createdAt)}
                </div>
              </div>
            ))}
            {jobs.length === 0 && <p className="py-3 text-[13px] text-[#95A79C]">No jobs yet.</p>}
          </div>
      ...
      <div className="mt-6">
        ...
        {records.length === 0 ? (
          <div className="border border-[#183A2B] bg-[#0C2018] p-10 text-center">
            ...
            <p className="mt-2 text-[13px] text-[#95A79C]">Imports appear here for approval before they can go live.</p>
          </div>
        ) : (
