...
export default async function ApprovalsPage() {
  ...
  return (
    <div>
      ...
      {listings.length === 0 ? (
        <Panel><p className="py-6 text-center text-[13.5px] text-[#8AA0B4]">Nothing to approve — the queue is clear.</p></Panel>
      ) : (
        <div className="grid gap-4">
          {listings.map((l) => (
            <div key={l.id} className="flex flex-wrap items-center justify-between gap-4 border border-[#1A3550] bg-[#0D2540] p-5">
              <div className="min-w-0">
                <Link href={`/property/${l.id}`} className="font-serif text-[20px] hover:text-[#C6A15C]">{l.property.title}</Link>
                <div className="mt-1 text-[12.5px] text-[#8AA0B4]">
              ...
              <div className="flex gap-2">
                <form action={approveListingAction}>
                  ...
                  <button className="border border-[#C6A15C] bg-[#C6A15C] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#0A1C33]">Approve</button>
                </form>
