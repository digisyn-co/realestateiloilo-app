...
export default async function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0A08] font-sans text-[#F4F0E6]">
      {/* header */}
      <header className="sticky top-0 z-50 border-b border-[#1D1B16] bg-[#0B0A08]/85 backdrop-blur">
        <div className="mx-auto flex h-[70px] max-w-[1360px] items-center gap-8 px-6">
          ...
          <nav className="hidden items-center gap-7 md:flex">
            {[["How it works", "#how"], ["Verified", "#verified"], ["Districts", "#districts"], ["Pricing", "#pricing"], ["Careers", "#careers"]].map(([l, h]) => (
              <a key={l} href={h} className="text-[9.5px] font-semibold uppercase tracking-[0.17em] text-[#F4F0E6]/70 hover:text-[#C9A227]">
                {l}
              </a>
            ))}
          </nav>
          <div className="ml-auto flex items-stretch">
            <Link href="/register" className="hidden items-center border border-[#33302A] px-4 text-[9.5px] font-semibold uppercase tracking-[0.16em] hover:border-[#C9A227] hover:text-[#C9A227] sm:flex">
              For brokers
            </Link>
            <Link href="/browse" className="flex items-center bg-[#C9A227] px-5 py-2.5 text-[9.5px] font-semibold uppercase tracking-[0.16em] text-[#0B0A08] hover:bg-[#F4F0E6]">
              Browse listings
            </Link>
          </div>
        </div>
      ...
      {/* hero */}
      <section className="relative h-[86vh] min-h-[560px] overflow-hidden bg-[#100E0B]">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#0B0A08]/70 via-[#0B0A08]/20 to-[#0B0A08]" />
        <div className="relative mx-auto flex h-full max-w-[1360px] flex-col justify-end px-6 pb-16">
          <h1 className="font-serif text-[clamp(46px,9vw,120px)] leading-[.9] tracking-[-.035em]">
            Every property
            <br />
            in <span className="italic text-[#C9A227]">Iloilo.</span> Verified.
          </h1>
          ...
          <div className="mt-8 flex flex-wrap">
            <Link href="/browse" className="bg-[#C9A227] px-7 py-4 text-[10px] font-bold uppercase tracking-[0.16em] text-[#0B0A08] hover:bg-[#F4F0E6]">
              Browse listings
            </Link>
            <Link href="/ai" className="border border-[#F4F0E6]/30 border-l-0 bg-[#0B0A08]/40 px-7 py-4 text-[10px] font-bold uppercase tracking-[0.16em] hover:border-[#C9A227] hover:text-[#C9A227]">
              ✦ Try AI search
            </Link>
          </div>
          <div className="mt-11 flex max-w-[860px] flex-wrap border-t border-[#F4F0E6]/15">
            {[[`${liveCount.toLocaleString()}`, "Live listings"], [`${brokerCount}`, "Verified brokers"], ["7", "City districts"]].map(([n, l]) => (
              <div key={l} className="flex-1 basis-[160px] pr-6 pt-5">
                ...
                <div className="mt-2 text-[9px] font-semibold uppercase tracking-[0.2em] text-[#8A8074]">{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section id="how" title="How it works" note="Describe what you want in plain words. The app does the filtering.">
        <div className="grid gap-8 pt-6 md:grid-cols-3">
          {[
            ...
            <div key={s.n} className="border-t border-[#1D1B16] pt-6">
              <div className="mb-3 flex items-baseline gap-4">
                <span className="font-serif text-[34px] text-[#C9A227] tabular-nums">{s.n}</span>
      ...
      <Section id="verified" title="Why verified matters">
        <div className="grid gap-px bg-[#1D1B16] md:grid-cols-4">
          {[
            ...
            <div key={v.t} className="bg-[#0B0A08] p-7">
      ...
      <Section id="districts" title="The districts" note="Seven districts and the neighbouring towns, each with its own character and price band.">
        <div className="grid grid-cols-2 gap-px bg-[#1D1B16] md:grid-cols-4 lg:grid-cols-6">
          {ILOILO_CITY_DISTRICTS.map((d, i) => (
            <Link key={d.slug} href={`/browse?city=${encodeURIComponent(d.name)}`} className="group relative aspect-[3/4] overflow-hidden bg-[#100E0B]">
              ...
              <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#0B0A08]/95" />
              <div className="absolute inset-x-4 bottom-4">
                <div className="mb-2 text-[9px] font-semibold uppercase tracking-[0.24em] text-[#C9A227]">No. 0{i + 1}</div>
      ...
      {featured.length > 0 && (
        <Section id="featured" title="Fresh on the register" note="A sample of verified homes live right now.">
          <div className="grid gap-4 pt-4 sm:grid-cols-2 lg:grid-cols-4">
            {featured.map((p) => (
              <Link key={p.id} href={`/property/${p.id}`} className="group block overflow-hidden rounded-card bg-[#0E0D0B] ring-1 ring-[#1D1B16]">
                <div className="relative aspect-[16/11] overflow-hidden bg-[#100E0B]">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-1 text-[12px] text-[#8A8074]">{p.area}</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      <Section id="pricing" title="Broker pricing" note="Buyers never pay. Verification is always free.">
        <div className="grid gap-px bg-[#1D1B16] md:grid-cols-3">
          {[
            ...
            <div key={p.name} className={`p-8 ${p.featured ? "bg-[#0E0D0B] border-t-2 border-[#C9A227]" : "bg-[#0B0A08]"}`}>
              {p.featured && <div className="mb-5 inline-block bg-[#C9A227] px-3.5 py-2 text-[8.5px] font-bold uppercase tracking-[0.2em] text-[#0B0A08]">Most popular</div>}
              <div className="mb-4 text-[9px] font-semibold uppercase tracking-[0.22em] text-[#C9A227]">{p.name}</div>
              <div className="font-serif text-[clamp(34px,3.8vw,46px)] leading-none tabular-nums">{p.price}</div>
              <div className="mb-6 mt-2 text-[9px] font-semibold uppercase tracking-[0.16em] text-[#6E655A]">{p.per}</div>
              <div className="border-t border-[#26231E]">
                {p.features.map((f) => (
                  <div key={f} className="flex items-baseline gap-3 border-b border-[#1D1B16] py-3">
                    <span className="text-[10px] text-[#C9A227]">✓</span>
                    <span className="text-[13.5px] text-[#F4F0E6]/78">{f}</span>
                  </div>
                ))}
              </div>
              <Link href="/register" className={`mt-6 block w-full py-4 text-center text-[9.5px] font-bold uppercase tracking-[0.16em] ${p.featured ? "bg-[#C9A227] text-[#0B0A08]" : "border border-[#C9A227] text-[#C9A227]"}`}>
      ...
      <section id="careers" className="mx-auto max-w-[1360px] px-6 pt-20">
        <div className="border-b border-[#C9A227] pb-6">
        ...
        <div>
          {["Senior Mobile Engineer", "Trust & Safety Associate", "Broker Success Manager", "Product Designer", "Field Photographer"].map((r) => (
            <div key={r} className="flex items-center gap-5 border-b border-[#1D1B16] py-5">
              <div className="flex-1 font-serif text-[24px]">{r}</div>
              <span className="text-[9px] font-semibold uppercase tracking-[0.16em] text-[#8A8074]">Iloilo City / Remote</span>
              <span className="font-serif text-[20px] text-[#C9A227]">→</span>
            </div>
          ))}
        </div>
      </section>

      <footer className="mt-20 border-t border-[#1D1B16]">
        <div className="mx-auto flex max-w-[1360px] flex-wrap gap-11 px-6 py-12">
          <div className="flex-1 basis-[280px]">
            ...
            <p className="max-w-[300px] text-[13px] leading-relaxed text-[#6E655A]">The property register for Iloilo City and Western Visayas. Every listing checked before it goes live.</p>
          </div>
          {[
            ...
            <div key={c.t} className="basis-[170px]">
              <div className="mb-4 text-[8.5px] font-semibold uppercase tracking-[0.2em] text-[#C9A227]">{c.t}</div>
              {c.links.map(([l, h]) => (
                <div key={l} className="py-1.5">
                  <Link href={h} className="text-[13px] text-[#8A8074] hover:text-[#C9A227]">
                    {l}
                  </Link>
                </div>
              ))}
        ...
        <div className="mx-auto flex max-w-[1360px] flex-wrap justify-between gap-4 border-t border-[#1D1B16] px-6 py-6">
          <span className="text-[8.5px] font-semibold uppercase tracking-[0.18em] text-[#4E4840]">© 2026 Real Estate Iloilo · Western Visayas, Philippines</span>
          <span className="text-[8.5px] font-semibold uppercase tracking-[0.18em] text-[#4E4840]">Sample site · Demonstration content</span>
        </div>
      </footer>
    </div>
  );
...
function Section({ id, title, note, children }: { id?: string; title: string; note?: string; children: React.ReactNode }) {
  return (
    <section id={id} className="mx-auto max-w-[1360px] px-6 pt-20">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#C9A227] pb-6">
        <h2 className="font-serif text-[clamp(32px,4.6vw,54px)] leading-none tracking-[-.03em]">{title}</h2>
        {note && <span className="max-w-[340px] text-right text-[13.5px] leading-relaxed text-[#8A8074]">{note}</span>}
      </div>
      {children}
    </section>
  );
