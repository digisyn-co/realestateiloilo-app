  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Recent leads" action={<Link href="/developer/leads" className="text-[12px] text-[#D6A84F]">View all</Link>}>
              ...
              {recentLeads.map((l) => (
                <div key={l.id} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <div className="text-[12px] text-[#95A79C]">{l.project.name}{l.agent ? ` · via ${l.agent.user.name}` : " · direct"}</div>
                  </div>
          ...
          <Link href="/developer/projects/new" className="mt-4 block border border-[#D6A84F] bg-[#D6A84F] px-4 py-3 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-[#05120C]">+ New project</Link>
        </Panel>
      </div>
    </div>
  );
...
function Stat({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between border border-[#183A2B] bg-[#05120C] px-4 py-3">
      <span className="text-[13.5px] text-[#95A79C]">{label}</span>
      <span className="font-serif text-[20px] tabular-nums text-[#D6A84F]">{value}</span>
    </div>
  );
}
