  ...
  return (
    <div>
      ...
      <Panel title={`Access requests · ${requests.length}`}>
        {requests.length === 0 ? (
          <p className="py-4 text-center text-[13.5px] text-[#95A79C]">No pending requests.</p>
        ) : (
            ...
            {requests.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center justify-between gap-3 border border-[#1C4635] bg-[#05120C] p-4">
                <div>
                  ...
                  <div className="text-[12px] text-[#95A79C]">{r.agent.company || "Independent"} · requested access to {r.project.name} · {timeAgo(r.createdAt)}</div>
                </div>
                <form action={resolveAgentAccessAction} className="flex items-center gap-2">
                  ...
                  <button name="decision" value="APPROVED" className="border border-[#D6A84F] bg-[#D6A84F] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#05120C]">Approve</button>
                  <button name="decision" value="REJECTED" className="border border-[#245140] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#95A79C] hover:border-[#C05B4A] hover:text-[#C05B4A]">Reject</button>
                </form>
              </div>
            ))}
          </div>
      ...
      <div className="mt-6">
        <Panel title={`Authorized agents · ${authorized.length}`}>
          {authorized.length === 0 ? (
            <p className="py-4 text-center text-[13.5px] text-[#95A79C]">No authorized agents yet.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[560px] text-left text-[13.5px]">
                ...
                <tbody>
                  {authorized.map((a) => (
                    <tr key={a.id} className="border-b border-[#183A2B]">
                      <td className="py-3 pr-4 font-medium">{a.agent.user.name}</td>
                      <td className="py-3 pr-4 text-[#95A79C]">{a.agent.company || "—"}</td>
                      <td className="py-3 pr-4 text-[#95A79C]">{a.project.name}</td>
                      <td className="py-3 pr-4 text-[#D6A84F]">{a.commissionPct != null ? `${a.commissionPct}%` : "—"}</td>
                      <td className="py-3">
                        <form action={resolveAgentAccessAction}>
                          <input type="hidden" name="id" value={a.id} />
                          <button name="decision" value="REVOKED" className="text-[11px] font-semibold text-[#95A79C] hover:text-[#C05B4A]">Revoke</button>
                        </form>
                      </td>
                    </tr>
                  ))}
