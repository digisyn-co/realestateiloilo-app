    ...
    <div className="max-w-[720px]">
      ...
      <div className="mt-6">
        <Panel title="Verification documents">
          <div className="grid gap-px bg-[#183A2B]">
            {[
              ...
              <div key={k} className="flex justify-between bg-[#05120C] px-4 py-3 text-[13.5px]">
                <span className="text-[#95A79C]">{k}</span>
                <span style={{ color: v === "Verified" ? "#6FB58F" : "#D6A84F" }}>{v}</span>
              </div>
            ))}
          </div>
        </Panel>
        ...
        <Panel title="Notifications">
          {["New lead alerts", "Price alerts", "Viewing alerts", "Weekly performance summary"].map((n) => (
            <label key={n} className="flex items-center justify-between border-b border-[#183A2B] py-3 text-[14px] last:border-0">
              <span>{n}</span>
              <input type="checkbox" defaultChecked className="h-4 w-4 accent-[#D6A84F]" />
            </label>
          ))}
        </Panel>
      </div>
