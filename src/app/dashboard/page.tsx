  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Panel title="Recent leads" action={<Link href="/dashboard/leads" className="text-[12px] text-[#C9A227]">View all</Link>}>
              ...
              {recentLeads.map((l) => (
                <div key={l.id} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <div className="text-[12px] text-[#8A8074]">{l.listing?.property.title || "General enquiry"}</div>
                  </div>
                  <span className="rounded-full border border-[#33302A] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#C9A227]">{l.stage}</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <Panel title="Quick actions">
          <div className="grid gap-3">
            <Link href="/dashboard/listings/new" className="block border border-[#C9A227] bg-[#C9A227] px-4 py-3.5 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-[#0B0A08]">
              + Add a listing
            </Link>
            <Link href="/dashboard/imports" className="block border border-[#33302A] px-4 py-3.5 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-[#F4F0E6] hover:border-[#C9A227] hover:text-[#C9A227]">
              Import listings
            </Link>
            <Link href="/dashboard/analytics" className="block border border-[#33302A] px-4 py-3.5 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-[#F4F0E6] hover:border-[#C9A227] hover:text-[#C9A227]">
              View analytics
            </Link>
          </div>
          <p className="mt-4 text-[12px] leading-relaxed text-[#8A8074]">Total portfolio: {listings} listing{listings === 1 ? "" : "s"}. New listings are reviewed by an admin before they go live.</p>
        </Panel>
      </div>
    </div>
  );
