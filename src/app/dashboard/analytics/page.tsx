            ...
            {days.map((d) => (
              <div key={d.label} className="flex flex-1 flex-col items-center gap-2">
                <div className="flex w-full flex-1 items-end">
                  <div className="w-full bg-[#C9A227]" style={{ height: `${(d.count / max) * 100}%`, minHeight: d.count ? 4 : 0 }} title={`${d.count} views`} />
                </div>
                <span className="text-[10px] text-[#8A8074]">{d.label}</span>
              </div>
            ))}
          </div>
        </Panel>
          ...
          <div className="divide-y divide-[#1D1B16]">
            {top.map((l) => (
              <div key={l.id} className="flex items-center justify-between py-2.5 text-[13.5px]">
                ...
                <span className="flex-none text-[#8A8074] tabular-nums">{l._count.views} views · {l._count.savedBy} saves</span>
              </div>
            ))}
            {top.length === 0 && <p className="py-4 text-center text-[13px] text-[#8A8074]">No data yet.</p>}
          </div>
        </Panel>
      </div>
      <p className="mt-4 text-[12px] text-[#8A8074]">Shares and viewing-request conversion are tracked in the data model and surface here as traffic grows.</p>
    </div>
  );
}
